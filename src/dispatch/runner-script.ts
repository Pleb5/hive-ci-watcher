 ...
 *
 * Copied verbatim from `budabit-pipelines-extension` so a watcher-triggered
 * run executes exactly what a human clicking "Run workflow" executes. It is a
 * static template: everything run-specific arrives through the 5100 `env`
 * tags, so its sha256 is identical across every run and every repo, which is
 * what makes the upload in `blossom.ts` a one-time cost.
 *
 * Note `HIVE_CI_BRANCH` carries a branch *or* a tag name —
 * `git clone --branch` accepts either, so tag builds need no change here.
 */
